...
module.exports = {
  doBackGroundJob: (event, cb) => {
    ...
    let arrySplitLength = Math.round(840 / expiry);
    console.log('arrySplitLangth', arrySplitLength);
    let remaingArry = [];
    if (driverArry.length >= arrySplitLength) {
      console.log('task is taking more than 15 mints');
    ...
    fetchTaskDetails(taskId).then((taskDetails) => {
      ...
      if (!_.isEmpty(taskDetails)) {
        return sendPushNotifications(driverArry, remaingArry, taskId, taskDetails, expiry, cb);
      } else {
...
function checkRemaingArryExistorNot(reamingArry, taskId, taskDetails, cb) {
  ...
  if (reamingArry.length) {
    ...
    gettaskDetails(taskId).then((res) => {
      ...
      console.log('clientId', JSON.stringify(taskDetails.clientId));
      if (taskDetails) {
...
//Favas checking on cherry-pick commends
